...
module "tvdlab-db21c" {
  ...
  defined_tags          = local.host_db21c_defined_tags           # Defined tags to tag the resources created
  ...
  host_ORACLE_ROOT      = var.host_db21c_ORACLE_ROOT              # default Oracle root / software folder 
  host_ORACLE_DATA      = var.host_db21c_ORACLE_DATA              # default Oracle data folder used to store datafiles
  host_ORACLE_ARCH      = var.host_db21c_ORACLE_ARCH              # default Oracle arch folder used to store archive logs and backups
  host_setup_folder     = var.host_db21c_setup_folder             # Host specific setup folder for post bootstrap scripts. Defaults to $path.module/cloudinit/templates/set_env_config.template.sh
  host_env_config       = var.host_db21c_env_config               # Host environment config script used to bootstrap host.
  host_enabled          = var.host_db21c_enabled                  # whether to create the compute instance or not.
  host_name             = var.host_db21c_name                     # Name portion of host
  host_image_id         = var.host_db21c_image_id                 # Provide a custom image id for the host or leave as OEL (Oracle Enterprise Linux).
  host_boot_volume_size = var.host_db21c_boot_volume_size         # Size of the boot volume.
  host_ocpus            = var.host_db21c_ocpus                    # The ocpus for the shape.
  host_memory_in_gbs    = var.host_db21c_memory_in_gbs            # The memory in gbs for the shape.
  host_volume_enabled   = var.host_db21c_volume_enabled           # add a block volume
  host_volume_size      = var.host_db21c_volume_size              # Size of the additional volume.
  host_volume_source    = var.host_db21c_volume_source            # Source block volume to clone from.
  host_private_ip       = var.host_db21c_private_ip               # Private IP for host.
  host_shape            = var.host_db21c_shape                    # The shape of compute instance.
...
# ------------------------------------------------------------------------------
locals {
  host_db21c_defined_tags = var.host_db21c_defined_tags != "" ? var.defined_tags : var.host_db21c_defined_tags
}
